...
dprintf(STDERR_FILENO, "Error: Can't read from file %s\n", argv[1]), exit(98);
	...
	print_info(&header->e_type, "Type:");
	print_info(&header->e_entry, "Entry:");

	free(header);

	if (close(fd))
		dprint(STDERR_FILENO, "Error: Can't close fd\n"), exit(98);
...
void print_info(unsigned char *data, char *label)
{
	...
	switch (label[0])
	{
		...
		case 'O':
			...
			data[EI_OSABI] == ELFOSABI_STANDALONE ? "Standalone (embedded) application" "Uknown OS");
